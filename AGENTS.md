...
- **`ghn-docs/ai/llms-full.txt`** — bảng **props** đầy đủ từng component (API Ant Design v6).
...
├── ghn-docs/ai/guide.md     # ⭐ hướng dẫn GHN đầy đủ (text, AI đọc) + components.json/llms-full.txt/recipes.json
└── src/
    ├── main.tsx             # <ConfigProvider theme={ghnTheme} locale={viVN}> + App  (BẮT BUỘC)
    ├── App.tsx              # màn hình khởi đầu — XOÁ và thay bằng UI của bạn
    ├── theme.ts             # ghnTheme: ThemeConfig (biến antd thành GHN)
    ├── components/
    ...
    └── styles/
        ...
        ├── tokens.css       # primitive + brand token
...
- [ ] App được bọc `<ConfigProvider theme={ghnTheme}>` — brand ra cam GHN, không phải xanh antd.
